...
| Component | Purpose |
| --- | --- |
| `Navbar` / `Footer` | Site chrome (in `layout.tsx`). One each — don't fork. The nav is a tall ABSOLUTE bar at the top of the page — it scrolls away (no fixed pill, no scroll transform; removed 2026-07-04). TALLER + RULED 2026-07-11 (h-28/h-36, full-bleed border-b hairline): the first screen had no frame — the nav's rule brackets its top as the industries band's rule closes its bottom. The footer's back-to-top anchor covers the return trip. `data-nav-light` markers remain on bone sections but are currently unused. |
...
RULED (2026-07-10, morning after): the NEW hero won — the old Deck hero is KEPT FOR REFERENCE at `/mockups/old-hero`, FROZEN as it stood on comparison night (client's request: do not delete, do not sync its body with the evolving site, do not update it at all). All night trials ratified: deep neutral ink `#0C0C0D` family, bone-dim `#C1B9B0`, full-size nav secondary, 1520 grid, hard-broken two-line H1. Hero is TYPE-LED with **THE DEAD-CORNER PORTRAIT** (settled 2026-07-10, round five of the hero-image saga — the four dead forms: device cluster, Deck showreel, and two panel options including a built tipped-in landscape, all killed for costing the masthead or the fold). The plate that lived costs neither: the H1 wraps to a short second line, and a 4:5 portrait (Rowen 5, the client's own mockup suite, with a REAL client site composited on the laptop screen) fills exactly the corner that leaves — top rides `var(--masthead-line)` (globals token sharing `.display-mega`'s size — never a copied clamp), bottom locks to the CTA row, right edge on the shell grid, width derived from height. ZERO type overlap, ZERO fold cost, H1 at full measure — lg+ only. MOBILE: the same plate in flow below the CTAs, right-anchored at 3/5 width — accepting, knowingly, that the LogoStrip slips just below the phone fold ("the fold bends before the whitespace does"). The masthead itself is untouchable: `display-mega` H1 with the italic accent, sans lede, flagship CTA pair (.btn-arrow + a ghost); any future hero image must occupy leftover space, never take it (the arithmetic: hero + strip ≈ 816px against 820–900px laptop viewports). Plate recipe: `docs/briefs/hero-plates.md`. Then: THE INDUSTRIES BAND (2026-07-11 — full-bleed under the hero: the three sector names in big Saol at `.industry-band-title` (vw-sized like .wordmark-giant, never ladder-sized), champagne ✦ ornaments between, each name linking to its /industries page — those SEO pages' first homepage links; nowrap, reader-scrollable on mobile) → The studio (statement + positioning prose — moved ABOVE the work 2026-07-09) → What we do (ruled .display rows, index-free — the formal stabiliser between two asymmetric neighbours) → Selected work (the page's only imagery: real captures in straight-cornered 16:10 frames, ruled captions, frontmatter `summary` — the work IS the proof) → (NO process section — it left the homepage entirely 2026-07-10 late after burning through THREE forms in one day: carousel, spine timeline, slim method strip — all killed by the client. /services owns the five steps in full, the homepage doesn't tease them, and a dedicated /process page was considered and REJECTED: no query targets it. Don't re-add a homepage process section without her asking) → Blog teasers (Carousel) → ContactCTA. An earn-its-place pass (2026-07-09) cut the hero disciplines list, the proof four-up, the service-row leads, the industries section and the CTA colophon rail — resist re-adding furniture without cause. The RECEIPTS STRIP (proof numbers briefly closing Selected work) was cut FOR GOOD 2026-07-10: audited against what the studio can stand behind, the numbers were positioning claims or too weak — a homepage stat must be SPECIFIC, VERIFIABLE and CLIENT-APPROVED, and none currently qualify. Real metrics go inside their case study (work frontmatter `metrics`) or on /about when they exist; no stats strip returns without numbers that clear that bar. A "Kind words" testimonial section sits after Selected work (returned 2026-07-09 — human proof after the work; imagery settled 2026-07-10): ONE quote at statement register, followed by the trust bar (LogoStrip — settled under the testimonial 2026-07-11 after two moves that day: one client speaks, then the roster corroborates; words → receipts); the big 4:5 slot carries a PORTRAIT MOCKUP at native orientation (RowenPhone 5 — the client's mobile site on a phone lying on travertine, the close plate's stone, so the page's plates read as one shoot; a landscape-recut was trialled first and killed — frames are used at native orientation only), with the human present as the CIRCULAR avatar chip in the ruled attribution (the corners rule's third exception; her full portrait is in reserve for /about). THE QUOTE stays visibly-marked placeholder until real client words + permission exist (we never draft quotes on a client's behalf). NOTHING performs — print stillness: nothing on the page moves unbidden (the trust-bar marquee was trialled & retired 2026-07-10; the strip is a STILL nowrap row, spread on md+, reader-scrollable on mobile). The never-reveal-above-the-fold rule (the observer's rootMargin excludes the bottom 10% of the viewport) applies to whatever sits in the first screen — currently the hero only; the strip reveals like its neighbours now it lives below the fold. Sanctioned motion: entrance reveals (pure fades, IN PLACE — the rise retired 2026-07-10: things develop where they sit, like ink coming up on paper; IMAGES take the SLOW tempo — 2.4s via .plate-develop / animate-fade-in-slow, 2026-07-11: prints develop after the ink sets), Lenis glide, the CTA exit-fade, and reader-driven rails — all deliberate keeps (2026-07-10 print audit). Deck, ghost marquee, ambient pools and the blend-weave hero stay parked.
