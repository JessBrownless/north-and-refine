...
⚠ DIRECTION IN FLUX (2026-07-11 night) — read this before touching the homepage. The client RESTORED THE LIVE SITE'S TOP onto today's body: the live nav (flagship arrow CTA, original height, no rule), the centred Deck-showreel hero (crown glow + grain scoped to it, track-in eyebrow at fixed 11px, `display-mega` H1 "Practices that Patients Trust", 21px md+ lede), and the scroll-scrubbed MANIFESTO — "A studio that treats the clinic's digital presence with the same care as the practice itself." at `.display`/100px on a 140vh sticky track with the ↳ Our story button. EXIT FADES are restored on EVERY homepage section (the 2026-07-09 homepage ban is repealed — each section dims to ink as it hands over; ExitFades drives them). The 2026-07-09→11 "flat editorial" top — dead-corner plate hero, industries caps band, statement-grid studio section, gradient-pools trial, accent-lands-last choreography — is STASHED AT COMMIT `37f0db9` (fully recoverable; its own commit message is the inventory), and the pure old hero stays frozen at `/mockups/old-hero`. **CANON RECONCILIATION PENDING:** the restored top runs live-era physics (Deck auto-cycle, `animate-fade-in-up`, sticky scrub) that the print-stillness rules below forbid — do NOT "fix" the homepage against the canon, and do NOT rewrite the canon, until the client settles the direction. Page order (2026-07-11): hero → manifesto → Selected work (moved ABOVE What-we-do to break the textiness; immediate-arrival, no top padding; mobile contact-sheet rail) → What we do (`.display` ledger rows, index-free) → Kind words (+ trust bar beneath it) → Blog (Carousel, Rowen scene plates) → ContactCTA. Type sweep 2026-07-11: manifesto `.display`, hero + close ledes 21px md+, `cta-label` 12px sitewide. Everything below the manifesto follows the current canon (ratio rules, card-titles, part titles, plates, slow image tempo, baseline locks).
