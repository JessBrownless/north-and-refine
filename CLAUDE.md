...
- `.shell-wide` — open, near-full-width container (max 1760px, lighter padding). Heroes, the **entire homepage** (every section runs the wide canvas so edges align — the "content to the edges" direction, decided 2026-07), and the work/[slug] article's IMAGE canvas (figures span it while kickers/copy stay grid-indented to a reading measure). Interior-page body copy still sits on the narrower `.shell` for a comfortable measure.
