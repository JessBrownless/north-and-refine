 ...
 */
export default function WorkCard({ project, index }: { project: WorkEntry; index: number }) {
  ...
  return (
    <Link href={`/work/${slug}`} className="group block reveal" style={{ transitionDelay: `${(index % 3) * 80}ms` }}>
      ...
      <div className="mt-5 flex items-baseline justify-between gap-4">
        <p className="overline text-champagne">{getSectorLabel(fm.sector)}</p>
        <p className="label text-clay">{fm.year}</p>
      </div>
      <h3 className="heading-md text-bone mt-2 transition-opacity group-hover:opacity-70">
        {fm.title}
      </h3>
      {fm.summary && <p className="body mt-2 text-bone-dim">{fm.summary}</p>}
