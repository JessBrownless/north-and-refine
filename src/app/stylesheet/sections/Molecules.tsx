    ...
    <>
      ...
      >
        ...
        <Sub
          ...
          note="A hero's buttons are not chosen from the four tiers by taste — the pair is fixed (2026-08-06). The primary is .btn-glass and the second action is .btn-ghost — on one baseline in the SPLIT heroes, STACKED on the CENTRED masthead (2026-08-08, the homepage: measured geometrically centred yet reading left-shifted, because a solid pill and a naked text link have unequal visual mass and cannot share a centred row; each centres on its own line instead). The reason glass belongs HERE and only here is material rather than decorative: it is 9% bone behind a blur, so it needs a lit ground, and a hero is the one slot on the site that has one, the warm HeroGlow. Below the fold the ground is flat and the same pill would be glass over nothing, which is why .btn-primary-dark is still the sitewide primary everywhere else. The secondary is added only where the page has a REAL onward step, never as a filler twin."
        />
