    ...
    <>
      ...
      >
        ...
        >
          ...
          <Beat n={3} name="ContactForm" what="Cols 7-12, spanning BOTH rows so it runs the full height of the left rail — which is what puts the first field level with the heading instead of below a masthead. Written BEFORE the details in the DOM, because on a phone the info must never precede the form. ⚠ IN THE HOUSE FORM PANEL, not a card: rounded-ui-lg, ONE bone/10 hairline, NO FILL — the same device the Start-a-project overlay uses. It replaced a gradient card on 2026-08-09 because that card carried ink-canvas + a contained HeroGlow, which is the SAME gradient language as the SectionGlow behind this band: not a distinct surface, just the ground repeated at another opacity. The hairline is bone/10 (≈#28241F) so it sits just UNDER the fields' rule-dark (#2F2820), mirroring the overlay's ink/25-panel-under-ink/35-fields relationship; bone/15 was rejected despite already existing because it computes brighter than rule-dark and would invert it. Netlify form project-enquiry." />
